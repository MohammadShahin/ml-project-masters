...
def train_model(dataset_path, eval_data_path, subset_name, output_path, model_name, num_epochs=3, train_batch_size=16,
                model_suffix='', data_file_suffix='', max_seq_length=256,
    ...
    logger.info("CUDA is available. Using GPU:", torch.cuda.get_device_name(0))
    train_examples = []
    with open(os.path.join(dataset_path, 'training_df{}.csv'.format(data_file_suffix)), encoding="utf-8") as fIn:
        reader = csv.DictReader(fIn, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        for row in reader:
            if loss == 'ContrastiveLoss':
                train_examples.append(InputExample(texts=[row['argument'], row['key_point']], label=int(row['label'])))
            else:
    ...
    evaluator = KeyPointEvaluator.from_eval_data_path(eval_data_path, subset_name, add_special_token, name='dev',
...
if __name__ == '__main__':
    # train_model('..\data', '..\data', 'our_valid', 'output', model_name)
    csv_file = '..\data\\training_df.csv'
    json_file = '..\..\corpora\parsed-corpora\kp_data.json'
